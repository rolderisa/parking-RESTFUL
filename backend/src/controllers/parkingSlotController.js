...
// Helper to parse ID from params (assuming Int ID)
const parseSlotId = (id) => {
  const parsed = parseInt(id);
  if (isNaN(parsed)) throw new Error('Invalid slot ID');
  return parsed;
};

// GET all parking slots with pagination & filters
export const getParkingSlots = asyncHandler(async (req, res) => {
  const { page, limit, type, isAvailable, slotNumber } = slotSearchSchema.parse(req.query);
  ...
  if (slotNumber) where.slotNumber = { contains: slotNumber };
...
export const getParkingSlotById = asyncHandler(async (req, res) => {
  const slotId = parseSlotId(req.params.id);
